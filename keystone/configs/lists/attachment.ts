        ...
        afterOperation: async ({ operation, item, context }) => {
          ...
          if (item.ocrAction === "run") {
            ...
            const orchestrators = await context.query.Orchestrator.findMany({
              where: {
                triggerEvent: {
                  equals: `attachment.ocrAction:${item.ocrAction}`,
                },
            ...
            for (const orchestrator of orchestrators) {
              await runOrchestrator({
                ...
                contextMap: {
                  /*
                      fileName and langage are examples of props being "extracted" and assigned in the 
                      orchestration step but note that its AGENT specific.

                      The agent REQUIRES that imagePath and language are present when it starts running
